...
export function Admin() {
  ...
  const [banModal, setBanModal]     = useState<BanModalState>({ open: false, user: null, reason: '', triggerMessage: '' });
  const [unbanModal, setUnbanModal] = useState<UnbanModalState>({ open: false, ban: null });
  ...
  useEffect(() => {
    ...
    moderationService.listUsers()
      .then(r => { if (!cancelled) setUsers(r.data.results); })
  ...
  const filteredBans = bans.filter(b =>
    `${b.user.full_name} ${b.user.email ?? ''}`.toLowerCase().includes(search.toLowerCase()),
  );
  ...
  const stats = [
    ...
    { label: 'Administradores',   value: users.filter(u => u.role === 'admin').length, icon: '🛡️' },
  ];
    ...
    <div className="admin">
      ...
      <aside className="admin__sidebar">
        ...
        <nav className="admin__nav">
          ...
          >
            ...
            Banimentos
            {bans.length > 0 && (
              <span className="admin__nav-badge admin__nav-badge--danger">{bans.length}</span>
            )}
          </button>
        ...
        <div className="admin__sidebar-footer">
          ...
          <button className="admin__back-link admin__back-link--danger" onClick={handleLogout}>
            Sair
          </button>
        </div>
      </aside>
      ...
      <div className="admin__main">
        ...
        <div className="admin__header">
          ...
          <Button variant="primary" size="lg" onClick={() => navigate('/criar-publicacao')}>
        ...
        <div className="admin__stats">
          {stats.map(s => (
            <div key={s.label} className={`admin__stat-card ${s.highlight ? 'admin__stat-card--danger' : ''}`}>
              <span className="admin__stat-icon" aria-hidden="true">{s.icon}</span>
              <div>
                      ...
                      <tr key={user.id}>
                        <td>
                          <div className="admin__user-cell">
                            <div className="admin__avatar admin__avatar--sm">{user.avatar_initial}</div>
                            <div>
                              <p className="admin__user-cell-name">{user.full_name}</p>
                              <p className="admin__user-cell-email">{user.email}</p>
                            </div>
                          </div>
                        </td>
                        <td>
                  ...
                  <div key={b.id} className="admin__ban-card">
                    <div className="admin__ban-card-header">
                      <div className="admin__user-cell">
                        ...
                        <div>
                          <p className="admin__user-cell-name">{b.user.full_name}</p>
                          <p className="admin__user-cell-email">{b.user.email}</p>
                        </div>
                      </div>
                      <div className="admin__ban-card-meta">
                        <p className="admin__cell-muted admin__cell-nowrap">
                          Banido em {new Date(b.created_at).toLocaleString('pt-BR')}
                        </p>
                        <p className="admin__cell-muted">por <strong>{b.banned_by.full_name}</strong></p>
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
        ...
        {tab === 'metricas' && (
          <section className="metrics" aria-labelledby="metrics-heading">
            <div className="metrics__topbar">
              <div>
                <h2 id="metrics-heading" className="metrics__title">Métricas</h2>
              ...
              <div className="metrics__controls">
                ...
                <div className="metrics__view-toggle" role="group" aria-label="Modo de visualização">
                  <button
                    type="button"
                  ...
                  >
                    <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
                      <path d="M2 13l3-4 3 2 5-7" strokeLinecap="round" strokeLinejoin="round" />
                ...
                <div className="metrics__period" role="group" aria-label="Selecionar período">
                  {PERIOD_LABELS.map(({ key, label }) => (
                    <button
                      ...
                      onClick={() => { setMetricsPeriod(key); loadMetrics(key); }}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <>
                ...
                <div className="metrics__hero-grid">
                  <HeroKpi
                    ...
                    delta={metrics.period_stats.new_subscribers - metrics.previous_period_stats.new_subscribers}
                  ...
                  <HeroKpi
                    ...
                    delta={metrics.period_stats.active_users - metrics.previous_period_stats.active_users}
                    deltaSuffix="comentaram ou curtiram"
                  />
                  <HeroKpi
                    label="Engajamento"
                    value={metrics.period_stats.new_comments + metrics.period_stats.new_likes}
                    delta={
                      (metrics.period_stats.new_comments + metrics.period_stats.new_likes) -
                      (metrics.previous_period_stats.new_comments + metrics.previous_period_stats.new_likes)
                    }
                ...
                {metricsView === 'dashboard' ? (
                  <>
                    <p className="metrics__section-eyebrow">Atividade e composição</p>
                  ...
                  <>
                    ...
                    <div className="metrics__secondary-grid">
                      <SmallStat label="Usuários cadastrados (total)" value={metrics.totals.users} />
                      <SmallStat label="Publicações (total)" value={metrics.totals.articles} />
                      <SmallStat label="Comentários (total)" value={metrics.totals.comments} />
                      <SmallStat label="Curtidas (total)" value={metrics.totals.likes} />
                      <SmallStat label="Novos usuários no período" value={metrics.period_stats.new_users} />
                      <SmallStat label="Novas publicações no período" value={metrics.period_stats.new_articles} />
                    </div>

                    {/* Per-article ranking with inline bar chart (Plausible/Posthog) */}
                    <p className="metrics__section-eyebrow">Publicações por performance</p>
                    {metrics.per_article.length === 0 ? (
                      <div className="metrics__empty">
                        <p className="metrics__empty-title">Sem publicações ainda</p>
                        <p className="metrics__empty-hint">
                          Crie sua primeira publicação para começar a coletar métricas.
                        </p>
                      </div>
                    ) : (
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setBanModal({ open: false, user: null, reason: '', triggerMessage: '' })}>
              Cancelar
            </Button>
            <Button
              variant="primary"
      ...
      >
        <div className="admin__ban-modal-body">
          <div className="admin__ban-modal-user">
            ...
            <div>
              <p className="admin__user-cell-name">{banModal.user?.full_name}</p>
          ...
          <div className="input-field">
            ...
            <textarea
              ...
              onChange={e => setBanModal(s => ({ ...s, triggerMessage: e.target.value }))}
              rows={3}
            />
          </div>

          <p className="admin__ban-warning">
            ⚠️ O usuário perderá acesso imediato à plataforma. Esta ação pode ser revertida a qualquer momento.
          </p>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setUnbanModal({ open: false, ban: null })}>
              Cancelar
            </Button>
            <Button variant="primary" onClick={confirmUnban} disabled={submitting}>
      ...
      >
        <div className="admin__ban-modal-body">
          <div className="admin__ban-modal-user">
            <div className="admin__avatar admin__avatar--banned">{unbanModal.ban?.user.avatar_initial}</div>
            <div>
              <p className="admin__user-cell-name">{unbanModal.ban?.user.full_name}</p>
              <p className="admin__user-cell-email">{unbanModal.ban?.user.email}</p>
            </div>
          </div>
          <p className="admin__unban-info">
            O usuário voltará a ter acesso completo à plataforma após o desbanimento.
          </p>
        </div>
      </Modal>
    </div>
...
function periodLabel(p: MetricsPeriod): string {
  switch (p) {
    ...
    case 'year':  return 'Últimos 365 dias';
  }
}
          ...
          return (
            <li key={a.slug} className="metrics__ranking-row">
              ...
              <div className="metrics__ranking-content">
                ...
                <span className="metrics__ranking-col metrics__ranking-num">{formatNumber(a.view_count)}</span>
                <span className="metrics__ranking-col metrics__ranking-num">{formatNumber(a.comment_count)}</span>
                <span className="metrics__ranking-col metrics__ranking-num">{formatNumber(a.like_count)}</span>
                <span className="metrics__ranking-col metrics__ranking-num metrics__ranking-engagement">{engagementPct}%</span>
              </div>
            </li>
          );
        })}
