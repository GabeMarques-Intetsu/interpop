...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const e = err as {
        response?: { status?: number; data?: { non_field_errors?: string[]; detail?: string } };
        request?: unknown;
        message?: string;
        code?: string;
      };
      const apiMsg = e?.response?.data?.non_field_errors?.[0] ?? e?.response?.data?.detail;
      let msg: string;
      if (apiMsg) {
        msg = apiMsg;
    ...
    >
      <form onSubmit={handleSubmit} className="auth-form" noValidate>
        ...
        <div className="auth-form__row">
          ...
          <Link to="/recuperar-senha" className="auth-link">Esqueceu a senha?</Link>
        </div>
      ...
      <p className="auth-switch">
        Não tem uma conta?{' '}
        <Link to="/cadastro" className="auth-link auth-link--strong">Criar conta grátis</Link>
      </p>
    </AuthLayout>
  );
}
