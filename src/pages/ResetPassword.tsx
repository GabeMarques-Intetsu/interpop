...
export function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = data?.token?.[0] ?? data?.new_password?.[0] ?? 'Não foi possível redefinir a senha.';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  if (success) {
    return (
      <AuthLayout heading="Senha redefinida!" subheading="Você será redirecionado para o login em instantes.">
        <div className="auth-success">
          ...
          <Link to="/login" className="btn btn--primary btn--md" style={{ marginTop: '1rem', display: 'inline-block' }}>
            Ir para o login
          </Link>
        </div>
      </AuthLayout>
